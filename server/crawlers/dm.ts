...
export class DmCrawler implements Crawler {
    ...
    getCanonical(rawItem: any, today: string): Item {
        ...
        const itemName = `${rawItem.brandName} ${rawItem.title}`;
        const description = itemName;
        const bio: boolean =
            rawItem.brandName === "dmBio" || (rawItem.name ? rawItem.name.startsWith("Bio ") | rawItem.name.startsWith("Bio-") : false);
        const unavailable = false;
        const productId = String(rawItem.gtin);
